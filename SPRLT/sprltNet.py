...
class SPRLT(nn.Module):
    def __init__(self, *, hidden_dim=128, layers=3, heads=12, channels=285, num_classes=21, head_dim=32, window_size=3,
                 relative_pos_embedding=True):  #channel=103 200 285  num_classes=9 16 21
        super().__init__()
    ...
    def forward(self, x):  # x: [B, W, H, C]        [64 1 30 15 15]
        #print(x.shape)
        x = rearrange(x.squeeze(1), 'b c h w -> b h w c')

        x = self.patch_partition(x) # [64, 15, 15, 128]
