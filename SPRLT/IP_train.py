...
def createImageCubes(X, y, windowSize, removeZeroLabels = True):
    ...
    # split patches
    patchesData = np.zeros((X.shape[0] * X.shape[1], windowSize, windowSize, X.shape[2]),dtype=np.float16)#  精度小降低OA
...
def create_data_loader():
    ...
    print('\n... ... create data cubes ... ...')
    X_pca, y_all = createImageCubes(X, y, windowSize=patch_size)
    ...
    X = X_pca.reshape(-1, patch_size, patch_size, X_pca.shape[3], 1) #pca_components
    Xtrain = Xtrain.reshape(-1, patch_size, patch_size, X_pca.shape[3], 1)#pca_components
    Xtest = Xtest.reshape(-1, patch_size, patch_size, X_pca.shape[3], 1)#pca_components
...
def train(train_loader, epochs):
    ...
    for epoch in range(epochs):#
        ...
        if ((epoch + 1) % 10 == 0 and (epoch + 1) >= 100):
